...
public class ImageTool implements Serializable {
	...
	public static void main(String[] args) {
		...
		bean.reduce("D:\\测试图片\\", "b17eca8065380cd7cf959dc2a044ad3459828111.jpg", 610, 610, true);// 压缩图片
		
		//bean.water("D:\\测试图片\\IMG_20130513_150847cc.jpgc", bean.outputFile);// 绘制水印
		
		//bean.rotate("D:\\opt\\", "/log/http_imgload2.jpg", 90, null);// 旋转图片
		
		//bean.zoom("D:\\opt\\", "/log/http_imgload2.jpg", 2, null);// 放大或缩小图片
		
		//bean.fontImg("D:\\测试图片\\IMG_20130513_150847cc.jpg", "中国", new Font("Atlantic Inline", Font.PLAIN, 18), Color.GREEN, 200, 100);// 把字符印到图片上
	}
	
	private static final long serialVersionUID = 1L;
	private String outputFile = "D:\\测试图片\\IMG_20130513_150847cc.jpg";
	...
	public String reduce(String base, String imgPath, int width, int height, boolean percentage) {
		try {
			...
			//System.out.println(base + imgPath);
			if(srcfile.length()/1024 >5){
	 ...
	 * @return
	 */
	public String reduce(File srcfile,String base, String imgPath, int width, int height, boolean percentage) {
		try {
			//File srcfile = new File(base + imgPath);
			//System.out.println(base + imgPath);
			if(srcfile.length()/1024 >70){
				BufferedImage src = ImageIO.read(srcfile);
				if (percentage) {
					double rate1 = ((double) src.getWidth(null)) / (double) width + 0.1;
					double rate2 = ((double) src.getHeight(null)) / (double) height + 0.1;
	...
	private String getNewPath(String imgPath) {
		...
		return filePrex  + imgPath.substring(filePrex.length());
	}
